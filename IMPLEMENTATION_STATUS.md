...
| 06 Control tower | Verified on hosted production | `8a2fd88` on `main` | 52 unit tests, production build, secret scan, 23 Playwright checks passed + 1 intentionally skipped duplicate mutation; GitHub run `29218981827` passed quality and browser jobs | Durable execution timeline, refresh recovery, interactive company/eval/review/incident/lineage screens, responsive roadmap and run telemetry, operator authorization, replay/reset, and neutral slate/teal visual system. The deployed guide has an ordered evidence-to-lineage walkthrough including Eval Campaigns and both Human Review gates; canonical `/evals` → `/reviews#release-gate` navigation was verified in the hosted browser. |
...
| 08 Demo hardening | Hosted workflow recovered through preview correction; paused at human release approval | `23a8193` | lint, typecheck, 52 unit tests, Next production build, secret scan, real remote preview browser runs | GitHub Actions-write 403 now falls back to repository dispatch. `ACTION-7E896E40F76A` resumed at preview evaluation without recreating planning/provider/build records. FEAT-0001 produced a real focus failure, correction commit `10ec2f2`, and passing rerun; FEAT-0002 passed. Both current previews score 100 and production is at `awaiting_release_approval`. |
