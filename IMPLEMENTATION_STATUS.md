...
| 08 Demo hardening | Recovery build deployed; fresh post-recovery workflow paused at its human feature gate | `2d5c0f5` | lint, typecheck, 52 unit tests, Next production build, 21 browser tests + 1 intentional skip, secret scan | Hosted code, route health, Vercel preview bypass, durable action schemas, unique session IDs, provider-write gates, and exact-target preview workflow are verified. The current hosted workflow remains at `awaiting_feature_approval`; no new external writes or release approval are claimed until the operator explicitly approves that gate. |
