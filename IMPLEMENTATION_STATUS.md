...
| 03 PM workflow | Complete | RUN-0100 / UX-0200 / EXT-0210 | `tests/agents-core.test.ts`, `tests/context-pack.test.ts` | Versioned company context pack, evidence-derived ranking, PM-owned implementation brief, UX and engineering-feasibility review runs, citations, risks, and approval pause/resume |
...
| 07 Integrations | Complete | mock/live adapters | `tests/connectors*.test.ts`, `/api/integrations/health`, `/delivery` | GitHub branch/commit/PR primitives, Slack logical-agent handoff thread and commands, Linear roadmap/status sync, Supabase workflow/external references, Langfuse trace/score, PostHog events, Inngest events, Vercel preview/production adapter, and sample-product traffic. |
...
| 08 Demo hardening | Complete | `pnpm demo` | `demo-verify`, secret scan, CI config, Playwright | Reset/run/verify scripts, product checkout recovery interaction, preview-target eval gate, live follow-along console, context browser, roadmap, troubleshooting, architecture diagram, attribution, release notes, and responsive coverage |
