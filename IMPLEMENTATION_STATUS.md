...
| 06 Control tower | Hosted baseline verified; synchronized walkthrough implemented and awaiting hosted verification | `publish-live` rebased on `bcf0312` | lint, typecheck, 54 unit/integration tests, Next production build, Playwright 25 passed + 1 intentionally skipped duplicate mutation | The hosted baseline passed GitHub quality/browser checks. The new walkthrough separates durable execution from a session-specific presentation chapter, preventing background completion from skipping the operator story. Manual navigation pauses auto-follow; refresh restores the same chapter. Live agent activity, provider records, approval packets, and the final delivery report poll authoritative state instead of page-local state. |
...
| 08 Demo hardening | Prior hosted workflow completed; synchronized presentation deployment pending | `publish-live` | lint, typecheck, 54 tests, build, browser route/interaction tests | The previous proof workflow reached production release. The new delivery report derives from existing completed sessions, while new sessions persist a presentation chapter independently from background execution. Primary UI destinations are real provider artifacts/dashboards; raw JSON is retained only in collapsed technical details. |
