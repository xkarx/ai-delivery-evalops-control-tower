...
| 08 Demo hardening | Complete | `pnpm demo` + hosted smoke | `demo-verify`, secret scan, CI config, Playwright, hosted operator/run check | Reset/run/verify scripts, two feature flags and product illustrations, preview-target eval gate, editable eval workbench, live follow-along console, guided onboarding, operator authorization, replay archive, serverless-compatible hosted demo actions, context browser, roadmap, troubleshooting, architecture diagram, attribution, release notes, and responsive coverage |
