...
import { persistStructuredRecord, recordActionReceipt } from "@/lib/durable-artifacts";
...
export async function POST() {
  ...
  try {
    ...
    await persistStructuredRecord("demo_sessions", sessionId, { workflowId, contextPackId: "CONTEXT-9001", status: "active", createdAt: at, sourceMode: process.env.INTEGRATION_MODE === "live" ? "live" : "mocked" });
    await persistStructuredRecord("context_packs", "CONTEXT-9001", { version: contextPack.version, manifest: contextPack.manifest, categories: contextPack.categories, evidence: contextPack.evidence, sourceMode: "synthetic" });
