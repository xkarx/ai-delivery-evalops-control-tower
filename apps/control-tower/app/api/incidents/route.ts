...
export async function POST(request: Request) {
  ...
  await writeArtifact("demoState", { ...data, incidents: [incident, ...data.incidents], lineage: [{ id: `LIN-${numericId}`, sourceType: "incident", sourceId: incident.id, relationship: "created_regression_case", targetType: "eval_case", targetId: evalCase.id, createdAt: at, metadata: { sessionId } }, ...data.lineage], activity: [{ at, type: "incident", title: "Production incident declared", detail: `${incident.id} created ${evalCase.id}.`, entityId: incident.id }, ...data.activity] }, sessionId);
  ...
  return NextResponse.json({ ok: true, sessionId, incident, evalCase, externalRefs, partial: externalRefs.length < 2 });
}
