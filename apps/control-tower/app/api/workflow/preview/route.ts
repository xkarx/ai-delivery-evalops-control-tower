...
export type PreviewBuild = { featureId: string; featureTitle?: string; branch: string; commitSha: string; commitUrl: string; pullRequestId: string; pullRequestUrl: string; deploymentUrl: string; deploymentId: string; externalDeploymentId?: string; sourceMode: string; createdAt: string; revision?: number; suite?: string; operationKey?: string };

function previewOperationKey(sessionId: string, featureId: string, revision: number, commitSha: string, suite: string): string {
  return [sessionId, featureId, revision, commitSha, suite].join(":");
}
...
function implementTrack(source: string, featureId: string, index: number, executionMode: "showcase" | "full_verification"): string {
  if (index === 0) {
    return source
      .replace(/const checkoutRecoveryEnabled = [^;]+;[^\n]*/, `const checkoutRecoveryEnabled = true; // ${featureId}: approved recovery guidance`)
      .replace("const focusRestorationEnabled = true;", executionMode === "full_verification" ? `const focusRestorationEnabled = false; // ${featureId}: first preview intentionally exposes the measured focus regression` : "const focusRestorationEnabled = true;")
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const workflow = await readArtifact<{ sessionId?: string; workflowInstanceId?: string; featureId: string; featureTitle?: string; featureTracks?: Array<{ featureId: string; featureTitle: string }>; executionMode?: "showcase" | "full_verification"; workflow?: { phase?: string; revision?: number } }>("workflow", sessionId);
    ...
    const suiteRevision = process.env.WORKFLOW_PREVIEW_SUITE ?? "product-preview-v1";
    const executionMode = workflow.executionMode ?? "showcase";
    ...
    if (body.correctBlocked && existing?.builds?.length) {
      ...
      const correctedRevision = workflow.workflow?.revision ?? 0;
      const correctedSuite = process.env.WORKFLOW_PREVIEW_SUITE ?? "product-preview-v1";
      const correctedBuild: PreviewBuild = { ...blocked, commitSha: commit.commitSha, commitUrl: commit.url, deploymentUrl: deployment.deployment.url, deploymentId: deployment.deployment.id, externalDeploymentId: deployment.externalId, sourceMode: deployment.sourceMode, createdAt: new Date().toISOString(), revision: correctedRevision, suite: correctedSuite, operationKey: previewOperationKey(sessionId, blocked.featureId, correctedRevision, commit.commitSha, correctedSuite) };
    ...
    const builds: PreviewBuild[] = [...(body.reconcile ? (existing?.builds ?? []).filter((build) => build.revision === revision && build.suite === suiteRevision) : [])];
    const reusable = new Set(builds.filter((build) => build.operationKey === previewOperationKey(sessionId, build.featureId, revision, build.commitSha, suiteRevision)).map((build) => build.featureId));
    for (const [index, track] of tracks.entries()) {
      if (reusable.has(track.featureId)) continue;
      const branch = `agent/${track.featureId.toLowerCase()}-${Date.now().toString(36)}`;
      await suite.codeHost.createBranch({ name: branch });
      const updated = implementTrack(source, track.featureId, index, executionMode);
