...
export async function POST(request: Request) {
  ...
  try {
    ...
    const stored = JSON.parse(await readFile(workflowPath, "utf8")) as { workflow: Parameters<typeof DeliveryWorkflow.hydrate>[0]; releaseApprovalId: string; featureId: string; featureTracks?: Array<{ releaseApprovalId: string }> };
