...
export async function POST(request: Request) {
  ...
  try {
    const stored = await readArtifact<{ sessionId?: string; workflow: Parameters<typeof DeliveryWorkflow.hydrate>[0]; featureId: string; featureTracks?: Array<{ featureId: string }> }>("workflow", sessionId);
    ...
    const preview = await readArtifact<{ builds?: Array<{ featureId: string; pullRequestId: string; commitSha: string }> }>("workflowPreview", sessionId);
    if (!preview?.builds?.length) throw new Error("Approved preview builds are missing.");
    const sync = await readArtifact<{ ticketRecords?: Array<{ externalId?: string; identifier: string; internalId: string }> }>("workflowSync", sessionId);
    ...
    await writeArtifact("workflow", { ...stored, sessionId, phase: snapshot.phase, workflow: snapshot, deployments, releaseMessage, completedTickets }, sessionId);
