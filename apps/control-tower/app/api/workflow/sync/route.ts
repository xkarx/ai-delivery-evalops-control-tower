  ...
  try {
    ...
    if (!state.trace) {
      try {
        ...
        try { await suite.trace.addScore({ traceId: trace.externalId, name: "delivery_candidate_created", value: true, comment: "Approved planning, agent outputs, tickets, and provider records are linked; preview evaluation remains pending." }); } catch (error) { state.errors.push(error instanceof ConnectorError ? `${error.provider}: ${error.message}` : "Trace score write failed."); }
    ...
    if (!state.workflowEvent) {
      try {
        const event = await suite.workflow.emit({ id: `delivery-${workflow.workflowInstanceId ?? workflow.sessionId ?? Date.now()}`, name: "dailycart/workflow.delivery_started", data: { sessionId: workflow.sessionId, workflowId: workflow.workflowInstanceId, featureId: workflow.featureId, ticketIds: data.tickets.filter((ticket) => ticket.featureId === workflow.featureId).map((ticket) => ticket.id), phase: "evaluation" } });
    ...
    try {
      ...
      await persistStructuredRecord("workflow_runs", workflow.workflowInstanceId ?? workflow.workflow?.id ?? `workflow-${Date.now()}`, { workflowType: "dailycart_delivery", sessionId, status: "running", featureId: workflow.featureId, phase: "evaluation", ticketIds: state.ticketRecords.map((record) => record.internalId), traceUrl: state.trace?.url }, sessionId);
