...
function sendEvent(event: "session_started" | "product_viewed" | "search_used" | "cart_added" | "checkout_started" | "checkout_completed", properties: Record<string, string | number | boolean | null>) {
  void fetch("/api/product/events", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: eventId(event), event, customerId, timestamp: new Date().toISOString(), properties, sourceMode: "simulated" }) }).catch(() => undefined);
}
...
export function ProductClient() {
  ...
  useEffect(() => { sendEvent("session_started", { surface: "dailycart-product" }); }, []);
  ...
  function add(product: Product) {
    ...
    sendEvent("product_viewed", { productId: product.id, productName: product.name });
    sendEvent("cart_added", { productId: product.id, price: product.price });
  }

  function beginCheckout() {
    if (!cart.length) return;
    setCheckout(true);
    sendEvent("checkout_started", { itemCount, total: Number(total.toFixed(2)) });
  }

  function completeCheckout() {
    setComplete(true);
    sendEvent("checkout_completed", { itemCount, total: Number(total.toFixed(2)) });
  }
