import { Activity, Filter, MousePointerClick, TrendingUp, Users } from "lucide-react";
import { PageHeading } from "@/app/ui/page-heading";
import { loadDemoState } from "@/lib/load-demo-state";
import { loadProductEvents } from "@/lib/load-product-events";
import { TrafficAction } from "./traffic-action";
...
export default async function AnalyticsPage() {
  ...
  return (
    <div className="page-container">
      <PageHeading eyebrow="Product intelligence" title="Product analytics" description="Deterministic traffic, persistent customers, funnel behavior, exposure, and release outcomes." actions={<><button className="button secondary"><Filter size={15} /> Segment</button><TrafficAction /></>} />
      ...
      <div className="analytics-layout"><section className="panel timeseries-card"><div className="section-title"><div><p className="eyebrow">Adoption trend</p><h2>Recovery success</h2></div><span className="source-label">Simulated events</span></div><div className="timeseries"><div className="y-labels"><span>100%</span><span>75%</span><span>50%</span><span>25%</span><span>0%</span></div><div className="plot"><div className="grid-lines"><i/><i/><i/><i/></div><svg viewBox="0 0 520 180" preserveAspectRatio="none" aria-label="Recovery success trend"><defs><linearGradient id="area" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#7656e8" stopOpacity=".28"/><stop offset="1" stopColor="#7656e8" stopOpacity="0"/></linearGradient></defs><path d={`M ${days.map((v, i) => `${(i / (days.length - 1)) * 520},${180 - (v / 100) * 160}`).join(" L ")} L 520,180 L 0,180 Z`} fill="url(#area)"/><polyline points={days.map((v, i) => `${(i / (days.length - 1)) * 520},${180 - (v / 100) * 160}`).join(" ")} fill="none" stroke="#6d4aff" strokeWidth="3" vectorEffect="non-scaling-stroke"/></svg><div className="release-marker"><span>DEP-0001</span></div></div></div><div className="chart-footer"><span>Jun 27</span><span>Jul 4</span><span>Jul 10</span></div></section><section className="panel segment-card"><div className="section-title"><div><p className="eyebrow">Experiment</p><h2>Exposed vs baseline</h2></div></div><div className="segment-bars"><div><span>Recovery guidance</span><b>79%</b><div><i style={{ width: "79%" }}/></div></div><div><span>Baseline</span><b>72%</b><div><i style={{ width: "72%" }}/></div></div></div><div className="uplift"><TrendingUp size={19}/><div><b>+6.8 percentage points</b><p>95% interval: +3.1 to +10.2 · simulated</p></div></div></section></div>
