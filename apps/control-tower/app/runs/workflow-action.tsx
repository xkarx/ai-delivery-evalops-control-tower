...
const phaseEstimate: Record<string, string> = {
  ...
  correcting_preview: "Usually 2–5 minutes", awaiting_feature_approval: "Waiting for your decision", awaiting_release_approval: "Waiting for your decision"
};
...
export function WorkflowAction() {
  ...
  useEffect(() => {
    ...
    if (action.status === "queued" || (action.status === "running" && heartbeatAge > 45_000)) startRecoveryWorker(action.actionId);
  ...
  async function execute(command: WorkflowCommand) {
    ...
    try {
      ...
      if (payload.actionId) startRecoveryWorker(payload.actionId);
  ...
  const stalled = Boolean(action && ((action.status === "queued" && heartbeatAge > 15_000) || (action.status === "running" && heartbeatAge > 45_000)));
