...
export default async function RunsPage() {
  ...
  const reviewArtifact = await readArtifact<{ agentEvals?: Array<{ id: string; runId: string; criterion: string; grader: string; score: number; passed: boolean; rationale: string; mode: string }> }>("workflowReviews");
  ...
  return (
    <div className="page-container">
      ...
      <div className="runs-layout">
        <section className="panel run-table-panel" id="agent-activity"><div className="section-title"><div><p className="eyebrow">Execution history</p><h2>Recorded runs</h2></div><span className="source-label">{runtimeMode === "live" ? "Live provider path" : "Simulated execution"}</span></div><div className="run-table"><div className="run-table-head"><span>Run</span><span>Agent</span><span>Status</span><span>Telemetry</span><span /></div>{data.runs.map((run) => { const agentLabel = run.agent === "engineering_feasibility" ? "engineering feasibility" : run.agent; const evaluations = agentEvals.filter((evaluation) => evaluation.runId === run.id); return <details key={run.id} id={`run-${run.id}`}><summary><span className="mono-id">{run.id}</span><span className="agent-cell"><i>{agentLabel.slice(0, 2).toUpperCase()}</i>{agentLabel}</span><StatusPill status={run.status} /><span className="telemetry"><b>{(run.latencyMs / 1000).toFixed(1)}s</b><small>${run.costUsd.toFixed(3)}</small></span><ChevronRight size={15} /></summary><div className="trace-detail"><div className="trace-head"><span><Route size={14} /> Trace {run.traceId}</span><b>{run.steps.length} steps · {run.retries} retries</b></div><div className="run-provenance"><span><b>Skill</b>{run.skillId ?? "legacy-role-contract"} · v{run.skillVersion ?? "1.0.0"}</span><span><b>Context</b>{run.contextPackId ?? "not attached"}</span><span><b>Evidence</b>{run.citedEvidenceIds?.join(", ") || "none"}</span><span><b>Tools</b>{run.toolCalls?.map((tool) => `${tool.provider}:${tool.name}`).join(", ") || "deterministic contract"}</span></div>{run.reasoningSummary && <p className="trace-reasoning"><b>Reasoning summary</b> {run.reasoningSummary}</p>}{evaluations.length > 0 && <section className="agent-eval-list" aria-label={`Evaluations for ${run.id}`}><b>Agent-output evaluations</b>{evaluations.map((evaluation) => <article key={evaluation.id}><StatusPill status={evaluation.passed ? "passed" : "failed"} /><div><strong>{evaluation.criterion}</strong><p>{evaluation.rationale}</p><small>{evaluation.grader} · {evaluation.mode} · {evaluation.score}/100</small></div></article>)}</section>}{run.steps.map((step, index) => <div className="trace-step" key={`${run.id}-${step.name}`}><span>{index + 1}</span><div><b>{step.name}</b><p>{step.detail}</p></div><small>{step.durationMs}ms</small><StatusPill status={step.status} /></div>)}</div></details>; })}</div></section>
