...
async function loadSyncRecords(): Promise<SyncRecord[]> {
  try {
    const root = path.resolve(process.cwd(), "../..");
    const parsed = JSON.parse(await readFile(path.resolve(root, "artifacts/linear-delivery-sync.json"), "utf8")) as { records?: SyncRecord[] };
...
export default async function DeliveryPage() {
  ...
  const synced = await loadSyncRecords();
  ...
  return <div className="page-container delivery-page">
    ...
    <p className="delivery-note">Statuses are sourced from the deterministic delivery state. Sync creates or updates Linear records with feature, PRD, evidence, owner, dependency, and workflow-status metadata; failed provider writes remain visible as errors.</p>
  </div>;
}
