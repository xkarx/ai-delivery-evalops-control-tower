...
async function bindAction(identity: Identity): Promise<void> {
  ...
  if (workflow) await writeArtifact("workflow", { ...workflow, sessionId: identity.sessionId, workflowInstanceId: identity.workflowId, activeActionId: identity.actionId });
}

async function completeStep(actionId: string, input: { id: string; label: string; detail: string; progress: number; phase: string; agent?: string; skillId?: string; provider?: string }): Promise<void> {
  ...
  await recordActionStep(actionId, step, { status: "running", progress: input.progress, phase: input.phase, message: input.detail, nextAction: "The workflow continues automatically until the next human decision." });
}
...
async function waitForPreviews(request: Request, identity: Identity): Promise<void> {
  for (let attempt = 1; attempt <= 36; attempt += 1) {
    ...
    if (statuses.length && statuses.every((item) => item.state === "READY")) {
      await completeStep(identity.actionId, { id: "preview-ready", label: "Preview deployments ready", detail: "Vercel reports every feature preview READY.", progress: 74, phase: "preview_ready", provider: "vercel" });
      return;
    }
    ...
    await updateAction(identity.actionId, { status: "running", phase: "waiting_vercel", progress: 68, message: `Waiting for Vercel previews (${attempt}/36).`, nextAction: "Browser evaluation starts when every preview is READY." });
...
export async function executeWorkflowActionDirect(request: Request, identity: Identity): Promise<WorkflowAction> {
  const original = await getAction(identity.actionId);
  if (!original) throw new Error(`ACTION_NOT_FOUND: ${identity.actionId} does not exist.`);
  await updateAction(identity.actionId, { status: "running", attempts: original.attempts + 1, phase: "starting", progress: 2, message: "Execution worker started.", nextAction: "Agent activity will appear here as each step completes." });
  try {
    if (identity.command === "analyze") {
      await completeStep(identity.actionId, { id: "context", label: "Company context retrieved", detail: "Loaded the versioned evidence pack and validated its references.", progress: 12, phase: "context", agent: "Context Retrieval Agent", skillId: "context-retrieval" });
      const current = await getAction(identity.actionId);
      ...
      await updateAction(identity.actionId, { status: "running", phase: "agent_research", progress: 22, message: "Research Agent is synthesizing interview evidence.", nextAction: "Seven visible agent and evaluation stages run before the feature-approval gate.", steps: [...(current?.steps ?? []), ...queuedAgents] });
    ...
    if (identity.command === "approve_feature" || identity.command === "retry") {
      if (identity.command === "approve_feature") {
        ...
        await bindAction(identity);
        await completeStep(identity.actionId, { id: "planning", label: "Delivery plan created", detail: "Approved scope was decomposed into workstreams, dependencies, owners, risks, and readiness checks.", progress: 28, phase: "planning", agent: "TPM", skillId: "implementation-planning" });
        await callRoute(request, "/api/workflow/sync", { method: "POST" }, identity);
        await completeStep(identity.actionId, { id: "providers", label: "Delivery records synchronized", detail: "Linear, Slack, Langfuse, Supabase, and Inngest references were recorded.", progress: 42, phase: "provider_sync", provider: "provider adapters" });
        await callRoute(request, "/api/workflow/preview", { method: "POST", body: JSON.stringify({ reconcile: true }) }, identity);
        await completeStep(identity.actionId, { id: "build", label: "Parallel product builds started", detail: "Feature branches, commits, pull requests, and preview deployments were created or reused.", progress: 62, phase: "building_preview", agent: "Engineering", skillId: "code-implementation" });
      } else {
        ...
        await bindAction(identity);
        await completeStep(identity.actionId, { id: "resume", label: "Resumed failed preview evaluation", detail: "Reusing the approved plan, provider records, pull requests, and READY Vercel previews without duplicating earlier work.", progress: 62, phase: "preview_ready", agent: "EvalOps", skillId: "release-readiness" });
      }
      ...
      if (!evaluation.allPassed) {
        ...
        await completeStep(identity.actionId, { id: "blocked", label: "Critical preview eval blocked release", detail: "Engineering is applying the measured focus-restoration correction.", progress: 80, phase: "correcting_preview", agent: "EvalOps + Engineering", skillId: "release-readiness" });
    ...
    await updateAction(identity.actionId, { status: "failed", phase: "failed", message: "Workflow action failed.", nextAction: "Inspect the exact failure and retry this step.", error: { code: detail.split(":")[0] || "WORKFLOW_ACTION_FAILED", detail, retryable: true } });
    throw error;
  }
}
