...
import { acquireExecutionLease, getAction, recordActionStep, releaseExecutionLease, updateAction } from "@/lib/workflow-actions";
...
type WorkflowActionEvent = { actionId: string; sessionId: string; workflowId: string; command: WorkflowCommand; rationale?: string; executionMode?: "showcase" | "full_verification" };
...
async function callRoute(path: string, init: RequestInit = {}, identity?: { sessionId: string; workflowId: string; actionId: string; executionMode?: "showcase" | "full_verification" }): Promise<Record<string, unknown>> {
  const response = await fetch(`${appUrl()}${path}`, {
    ...init,
    headers: { "content-type": "application/json", authorization: `Bearer ${process.env.DAILYCART_OPERATOR_PASSCODE ?? ""}`, ...(identity ? { "x-dailycart-session-id": identity.sessionId, "x-dailycart-workflow-id": identity.workflowId, "x-dailycart-action-id": identity.actionId, "x-dailycart-execution-mode": identity.executionMode ?? "showcase" } : {}), ...(init.headers as Record<string, string> | undefined) }
  });
...
async function waitForPreviewReadiness(actionId: string, identity: WorkflowActionEvent): Promise<void> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (statuses.some((item) => ["ERROR", "CANCELED"].includes(item.state ?? ""))) throw new Error("PREVIEW_DEPLOYMENT_FAILED: A Vercel preview deployment failed before evaluation.");
    ...
    await updateAction(actionId, { status: "running", phase: "waiting_vercel", progress: 68, message: `Waiting for Vercel previews (${attempt}/${maxAttempts})…`, nextAction: "Preview evaluation starts automatically when every deployment is READY." }, identity.sessionId);
...
export const executeWorkflowAction = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const identity = { sessionId: data.sessionId, workflowId: data.workflowId, actionId: data.actionId, executionMode: data.executionMode ?? "showcase" as const };
    ...
    const leaseOwner = `inngest:${event.id ?? data.actionId}`;
    const lease = await acquireExecutionLease(data.actionId, data.sessionId, leaseOwner);
    if (!lease.acquired || !lease.lease) return lease.action;
    ...
    try {
      ...
      if (data.command === "approve_feature" || data.command === "retry") {
        ...
        await progress(data.actionId, data.sessionId, { id: "eval", label: "Preview evaluations passed", detail: `${data.executionMode === "showcase" ? "The current preview commit" : "Both current preview commits"} passed critical release checks.`, progress: 100, phase: "awaiting_release_approval", agent: "EvalOps", skillId: "release-readiness" });
      ...
      await updateAction(data.actionId, { status: "failed", phase: timedOut ? "waiting_vercel" : "failed", message: timedOut ? "Vercel preview readiness timed out without marking the build failed." : "Workflow action failed.", nextAction: "Retry to resume polling from the existing preview deployments.", error: { code: detail.split(":")[0] || "WORKFLOW_ACTION_FAILED", detail, retryable: true } }, data.sessionId);
      throw error;
    } finally {
      await releaseExecutionLease(data.actionId, data.sessionId, leaseOwner, lease.lease.token).catch(() => undefined);
    }
  }
);
