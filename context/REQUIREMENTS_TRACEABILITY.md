...
| R050 | Usable operator unlock boundary | Control plane + deployment security | Implemented and locally verified — the global KO control opens an accessible operator panel, posts only the entered passcode to `/api/operator/auth`, refreshes cockpit authorization after success, and turns `OPERATOR_AUTH_REQUIRED` into an actionable unlock state. Verified by lint, typecheck, 57 unit/integration tests, production build, and Playwright desktop/mobile interaction coverage; hosted promotion remains pending. |
