...
| R051 | Persisted visible operator authorization | Control plane + deployment security | Implemented and locally verified — `GET /api/operator/auth` reports only authorization state, never the passcode; the application shell hydrates that state on load and after authentication so refresh/navigation retains the visible unlocked state. Typecheck, lint, 65 tests, production build, secret scan, and focused desktop/mobile Playwright checks pass. |
