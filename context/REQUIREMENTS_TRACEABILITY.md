...
| R045 | Durable Inngest execution proof | Orchestration | Recovery implemented — session-serialized `execute-guided-workflow-action` persists phase progress, heartbeats, retries, failures, and human waits. Fresh hosted execution evidence awaits feature approval. |
| R046 | Refresh-safe guided workflow recovery | Control plane + orchestration | Implemented and hosted route-verified — durable action/status APIs, session-scoped IDs, idempotency, phase-aware CTA, guide recovery, and persisted preview/provider hydration; current hosted phase is honestly reported as `awaiting_feature_approval` |
