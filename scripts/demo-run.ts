...
import { analyzeProductEvidence, createPlanningAndDeliveryLineage, createTpmPlan, executeIndependentEngineeringWorkstreams } from "../packages/agents/src/index";
...
const plan = createTpmPlan(recommended, { runId: "RUN-0101", now: "2026-07-10T16:15:00.000Z", ticketStartOrdinal: 101 });
...
state.runs = [pm.run, plan.run, ...workstreams.map((record) => record.run), ...state.runs.filter((run) => run.agent !== "pm" && run.agent !== "engineering")];
...
state.lineage = createPlanningAndDeliveryLineage({ evidenceIds: recommended.evidenceIds, featureId: recommended.id, decisionId: "DEC-0001", prdId: plan.prd.id, tickets: plan.tickets, workstreams, createdAt: "2026-07-10T16:30:00.000Z" });
